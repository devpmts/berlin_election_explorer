...
public class KolporitExpressionParser {
	...
	private Map<String, Integer> electionResults;

	public KolporitExpressionParser(String expression, Map<String, Object> variableDefinitions,
			Map<String, Integer> electionResults) {
		this.variableDefinitions = variableDefinitions;
		this.electionResults = electionResults;
		this.expression = expression;
	}

	public static double parse(String expression, Map<String, Object> variableDefinitions,
			Map<String, Integer> electionResults) throws ParseException {
		if (expression == null) {
			return -1d;
		}
		return new KolporitExpressionParser(expression, variableDefinitions, electionResults)
				.parse();
	}
	...
	private void parseVariableDefinition(int i, StandardEvaluationContext evaluationContext) {
		...
		if (definition instanceof String) {
			...
			System.err.println(name + ": " + parsed);
		} else {
	...
	private double parseExpression(String expression, StandardEvaluationContext evaluationContext) {
		DevsLogger.log("parsing expression: " + expression);
		SpelExpressionParser spelExpressionParser = new SpelExpressionParser();
		Expression spelExpression = spelExpressionParser.parseExpression(expression);
		try {
			double parsed = spelExpression.getValue(evaluationContext, Double.class);
			DevsLogger.log("Parsed '" + expression + "' to '" + parsed + "'");
			return parsed;
	...
	private void addParserVariablesFromMap(Map<String, ?> variables,
	...
	private void setParserVariable(StandardEvaluationContext evaluationContext, String variable,
			Object definition) {
		if (variable == null || definition == null) {
			DevsLogger.log("Warning! Variable " + variable + "=" + definition + " has null");
			return;
		}
