...
public class KolporitDataImporter {
	...
	public void importData() {
		DevsLogger.log("clearing repos");
		clearRepositories();
		importPartys();
		DevsLogger.log(partyRepository.count() + " parties imported.");
		importBezirke();
		DevsLogger.log(bezirkRepository.count() + " bezirke imported.");
		importWahlbezirke();
		DevsLogger.log(wahlbezirkRepository.count() + " wahlbezirke imported.");
		importElection();
	}
	...
	private void importWahlbezirke() {
		DevpmtsUtil.readAndPersistFromCsv(WAHLBEZIRKE_FILE.getPath(), values -> {
